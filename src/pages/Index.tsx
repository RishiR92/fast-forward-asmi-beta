...
const Index = () => {
  ...
  // Four demo examples - each fits on single screen
  const demoExamples = [
    {
      ...
      messages: [
        ...
        { type: "asmi", text: "Well well Alex, you are absolutely crushing it\n\n• Serial entrepreneur & CEO at TechFlow\n• Boston Marathon finisher (3:12 best time)\n• 5× founder with 2 successful exits\n• Stanford MBA, former McKinsey consultant", delay: 3000, timestamp: "9:16 AM" }
      ]
    },
    {
      title: "Email & Calendar Insights", 
      messages: [
        { type: "asmi", text: "Insights from your emails & calendar", delay: 2000, timestamp: "9:17 AM" },
        { type: "asmi", text: "📧 \"Need to finalize Q3 roadmap by Friday - pressure from board\"\n📧 \"Marketing strategy review 2 weeks overdue, Sophia asking for feedback\"\n📧 \"Steve Johnson following up on Acme Corp partnership - big opportunity\"", delay: 3000, timestamp: "9:17 AM" },
        { type: "asmi", text: "📅 Back-to-back meetings every day this week\n📅 3 investor calls scheduled for next week\n📧 \"Pat from Sequoia mentioned potential Series B introduction\"", delay: 3000, timestamp: "9:18 AM" }
      ]
    },
    {
      title: "Smart Scheduling",
      messages: [
        { type: "user", text: "Schedule a call with Steve for 3 PM tomorrow.", delay: 2000, timestamp: "9:20 AM" },
        { type: "asmi", text: "Call with Steve\n\n🕒 03:00 PM, Aug 8, 2025\n👤 Steve Johnson\n📋 Agenda: Catch up + next steps\n🔗 meet.google.com/xyz-1234", delay: 2500, timestamp: "9:20 AM" },
        { type: "user", text: "Confirmed", delay: 1500, timestamp: "9:21 AM" },
        { type: "asmi", text: "Meeting scheduled ✅", delay: 1500, timestamp: "9:21 AM" }
      ]
    },
    {
      title: "Meeting Prep & Advanced Tasks",
      messages: [
        { type: "user", text: "Help me prep for this meeting.", delay: 2000, timestamp: "9:25 AM" },
        { type: "asmi", text: "Meeting Prep: Steve Johnson\n\nObjective: Align on Q3 roadmap & secure Acme Corp partnership\n\nProfile:\n• VP Sales at Acme Corp (3 years)\n• Former director at Oracle\n• Key decision maker for vendor partnerships ($500K+ deals)", delay: 3000, timestamp: "9:25 AM" },
        { type: "user", text: "Book 1 hr dinner with Sophia to run me through marketing plan. Pick a quiet downtown SF spot.", delay: 2500, timestamp: "9:26 AM" },
        { type: "asmi", text: "Dinner with Sophia\n\n🕗 07:00 PM, Aug 8, 2025\n👤 Sophia Lee, BrightWave\n📍 The Rotunda at Neiman Marcus\n📋 Agenda: Marketing plan walkthrough", delay: 2500, timestamp: "9:27 AM" }
      ]
    }
  ];

  const currentMessages = demoExamples[currentDemo] || demoExamples[0];
  ...
  // Demo animation logic for 4 examples
  useEffect(() => {
    const currentExample = demoExamples[currentDemo];
    
    if (messageIndex < currentExample.messages.length - 1) {
      const timer = setTimeout(() => {
        setMessageIndex(prev => prev + 1);
      }, currentExample.messages[messageIndex].delay);
      
      return () => clearTimeout(timer);
    } else {
      // Reset to next example after all messages are shown
                  ...
                  {!isTransitioning && (
                    ...
                    >
                      <div className="bg-[#0B141A] rounded-t-3xl h-full flex flex-col">
                        ...
                        {/* Demo Title */}
                        <div className="p-3 bg-[#1A2A33] border-b border-gray-700">
                          <p className="text-[#5DFF9F] text-xs font-medium text-center">
                        ...
                        <div className="p-4 space-y-3 bg-[#0B141A] flex-1 overflow-y-auto">
                          {currentMessages.messages.slice(0, messageIndex + 1).map((message, index) => (
                            <motion.div
                              key={`demo-${currentDemo}-${index}-${demoKey}`}
                              initial={{ opacity: 0, scale: 0.8, y: 20 }}
                              animate={{ opacity: 1, scale: 1, y: 0 }}
                              transition={{ duration: 0.4, ease: "easeOut" }}
                              className={message.type === 'asmi' || message.type === 'system' ? 
                            ...
                            >
                              <p className="text-white text-sm whitespace-pre-line leading-relaxed">
                                {message.type === 'system' ? (
                                  <span className="text-[#5DFF9F] font-medium">{message.text}</span>
                                ) : (
                                  message.text
                                )}
                              </p>
